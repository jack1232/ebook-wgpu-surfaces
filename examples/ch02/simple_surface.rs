...
fn create_vertices(ss_data: sd::ISurfaceOutput) -> (Vec<Vertex>, Vec<Vertex>, Vec<u16>, Vec<u16>) {
    ...
    for i in 0..ss_data.positions.len() {       
        data.push(Vertex{position:ss_data.positions[i], normal:ss_data.normals[i], 
            color:ss_data.colors[i]});
        data2.push(Vertex{position:ss_data.positions[i], normal:ss_data.normals[i],
            color:ss_data.colors2[i]});
    }
    (data.to_vec(), data2.to_vec(), ss_data.indices, ss_data.indices2)
}
...
async fn new(
    ...
    wireframe_color:&str
        ...
        let vs_shader = init.device.create_shader_module(wgpu::include_wgsl!("shader_vert.wgsl"));        
        let fs_shader = init.device.create_shader_module(
            wgpu::include_wgsl!("directional_frag.wgsl"));
        ...
        init.queue.write_buffer(&light_uniform_buffer, 0, cast_slice(light_direction.as_ref()));
        init.queue.write_buffer(&light_uniform_buffer, 16, cast_slice(eye_position));
               
        // set specular light color to white
        let specular_color:[f32; 3] = [1.0, 1.0, 1.0];
        init.queue.write_buffer(&light_uniform_buffer, 32, cast_slice(specular_color.as_ref()));
        ...
        init.queue.write_buffer(&material_uniform_buffer, 0, cast_slice(material.as_ref()));
        ...
        Self {
            ...
            uniform_bind_groups: vec![vert_bind_group, frag_bind_group, vert_bind_group2,
                frag_bind_group2],
            uniform_buffers: vec![vert_uniform_buffer, light_uniform_buffer, material_uniform_buffer],
            view_mat,
            project_mat,
            msaa_texture_view,
            depth_texture_view,
                ...
                match keycode {
                    ...
                    VirtualKeyCode::LControl => {
                        self.simple_surface.surface_type = 
                            (self.simple_surface.surface_type + 1) % 3;                        
                        true
                    },
                    VirtualKeyCode::LAlt => {
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        let model_mat = ws::create_model_mat([0.0,1.0,0.0], [dt1.sin(), dt1.cos(), 0.0], [1.0, 1.0, 1.0]);
        ...
        self.init.queue.write_buffer(&self.uniform_buffers[0], 0, cast_slice(view_projection_ref));
        self.init.queue.write_buffer(&self.uniform_buffers[0], 64, cast_slice(model_ref));        
        self.init.queue.write_buffer(&self.uniform_buffers[0], 128, cast_slice(normal_ref));
        ...
        if self.recreate_buffers {
            ...
            for i in 0..2 {
                ...
                self.vertex_buffers[i] = self.init.device.create_buffer_init(
                ...
                self.index_buffers[i] = self.init.device.create_buffer_init(
                &wgpu::util::BufferInitDescriptor {
                    label: Some("Index Buffer"),
        ...
        let data = create_vertices(self.simple_surface.new());
        self.init.queue.write_buffer(&self.vertex_buffers[0], 0, cast_slice(&data.0));
        self.init.queue.write_buffer(&self.vertex_buffers[1], 0, cast_slice(&data.1));      
    }
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            let plot_type = if self.plot_type == 1 { "shape_only" } 
                else if self.plot_type == 2 {"wireframe_only"} else {"both"};
            ...
            if plot_type == "wireframe_only" || plot_type == "both" {
                ...
                render_pass.set_index_buffer(self.index_buffers[1].slice(..), wgpu::IndexFormat::Uint16);        
...
fn main() {
    ...
    window.set_title(&*format!("{}", "simple_surface"));

let mut state = pollster::block_on(State::new(&window, sample_count, colormap_name,
    wireframe_color));    
